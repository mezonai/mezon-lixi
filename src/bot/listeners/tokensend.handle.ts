...
export class ListenerTokenSend extends BaseQueueProcessor<TokenSentEvent> {
  ...
  async handleRecharge(tokenEvent: TokenSentEvent) {
    ...
    const botId = process.env.LIXI_BOT_ID;
    if (!botId) {
      console.error('LIXI_BOT_ID is not defined');
      return;
    }
  ...
  protected async processItem(tokenEvent: TokenSentEvent): Promise<void> {
    ...
    if (!botId) {
      throw new Error('LIXI_BOT_ID is not defined');
    }
    ...
    try {
      ...
      const successMessage = `💸Nạp ${tokenEvent.amount.toLocaleString('vi-VN')} mezon đồng thành công`;
      await user?.sendDM({
        t: successMessage,
