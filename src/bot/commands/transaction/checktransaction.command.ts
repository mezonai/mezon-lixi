  ...
  async execute(args: string[], message: ChannelMessage) {
    ...
    if (args[0] === 'admin') {
      ...
      const findBot = await this.userRepository.findOne({
        where: { user_id: process.env.LIXI_BOT_ID },
      });
      if (!findBot) return
  ...
  async processCheckTransaction(message: ChannelMessage) {
    ...
    if (!findTransaction) {
      ...
      if (
        ...
        transaction.receiver_id === process.env.LIXI_BOT_ID
      ) {
        const trans = {
        ...
        const users = await this.userRepository.find({
          where: [
            ...
            { user_id: process.env.LIXI_BOT_ID },
          ],
        });
        ...
        const botInfo = users.find(
          (user) => user.user_id === process.env.LIXI_BOT_ID,
        );
